...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(sensorTask, StartDefaultTask, osPriorityNormal, 0, 128);
  sensorTaskHandle = osThreadCreate(osThread(sensorTask), NULL);

  /* definition and creation of uiTask */
  osThreadDef(uiTask, StartTask02, osPriorityIdle, 0, 256);
  uiTaskHandle = osThreadCreate(osThread(uiTask), NULL);

  /* definition and creation of btTask */
  osThreadDef(btTask, StartTask03, osPriorityIdle, 0, 128);
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
    /* Keep-alive: PB12 + PB13 always HIGH */
    ...
    if (watch_data.imu_status)
    {
      ...
      Pedometer_Update(&watch_data.accel, &watch_data.gyro);
      watch_data.steps      = Pedometer_GetSteps();
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
    UI_DrawPage(PAGE_WATCH_FACE, &watch_data);
    osDelay(200);
  }
...
void StartTask03(void consat * argument)
{
  /* USER CODE BEGIN StartTask03 */
  for(;;)
  {
    BT_Process(&watch_data);
